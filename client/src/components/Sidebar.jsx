...
const Sidebar = ({ sidebar, setSidebar }) => {
  ...
  return (
    <div className={`w-60 bg-white border-r border-gray-200 
      ...
      top-14 bottom-0 z-20 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} 
      transition-all duration-300 ease-in-out`}>

      <div className='my-7 w-full px-4'>
        {/* User Info */}
        <img src={user?.imageUrl} alt="user" className='w-14 h-14 rounded-full mx-auto' />
        ...
        <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
          {navItems.map(({ to, label, Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `px-4 py-2.5 flex items-center gap-3 rounded-lg text-sm transition-colors
       ...
       <div className='w-full border-t border-gray-200 p-4 px-7 flexx items-center
       justify-between'>
